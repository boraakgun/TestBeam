...
void RawHitPlotter::analyze(const edm::Event& event, const edm::EventSetup& setup)
{
  ...
  CommonMode cm(m_emap, false , false);
  cm.Evaluate( hits );
  ...
  for ( auto hit : *hits ) {
    ...
    m_chipMap[module*100+chipID].cmn=cmMap[board];
	...
	*/
      case 0 : subHG=cmMap[board].fullHG[it]; subLG=cmMap[board].fullLG[it]; break;
      case 2 : subHG=cmMap[board].halfHG[it]; subLG=cmMap[board].halfLG[it]; break;
      case 3 : subHG=cmMap[board].mouseBiteHG[it]; subLG=cmMap[board].mouseBiteLG[it]; break;
      case 4 : subHG=cmMap[board].outerHG[it]; subLG=cmMap[board].outerLG[it]; break;
      case 5 : subHG=cmMap[board].mergedHG[it]; subLG=cmMap[board].mergedLG[it]; break;	
      }
